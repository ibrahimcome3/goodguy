...
if ($products_in_cart) {
    foreach ($products_in_cart as $item) {
        $inventory_item_id = $item['inventory_product_id'];
        $quantity = $item['quantity'];
        $product_details = $invt->get_product_details($inventory_item_id);
        if ($product_details) {
            if ($promotion->check_if_item_is_in_inventory_promotion($inventory_item_id)) {
                $subtotal += (float) $promotion->get_promoPrice_price($inventory_item_id) * (int) $quantity;
            } else {
                $subtotal += (float) $product_details['cost'] * (int) $quantity;
            }
        }
    }
}
            ...
            <div class="page-content">
                <div class="cart">
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-lg-9">
                                <form action="cart.php" method="post" class="update_cart_form">
                                    ...
                                    <table class="table table-cart table-mobile">
                                        ...
                                        <tbody>
                                            ...
                                            <?php } else { ?>
                                                    <?php
                                                    $product_obj = $p = new ProductItem();
                                                    foreach ($products_in_cart as $key => $item) {
                                                        $inventory_item_id = $item['inventory_product_id'];
                                                        $quantity = $item['quantity'];
                                                        $size = $item['size'];
                                                        $color = $item['color'];
                                                        $product_details = $invt->get_product_details($inventory_item_id);
                                                        if ($product_details) {
                                                            $product_id_cost = $inventory_item_id;
                                                            $product_name = $product_details['description'];
                                                            $product_price = $product_details['cost'];
                                                            $product_image = $invt->get_product_image($inventory_item_id);
                                                            if ($promotion->check_if_item_is_in_inventory_promotion($inventory_item_id)) {
                                                                $product_price = $promotion->get_promoPrice_price($inventory_item_id);
                                                            }
                                                            ...
                                                            ?>
                                                                    <tr>
                                                                        ...
                                                                        <td class="quantity-col">
                                                                            <div class="cart-product-quantity">
                                                                                <input type="number" name="quantity[<?= $key ?>]" class="form-control" value="<?= $quantity ?>" min="1" max="10" step="1" data-decimals="0" required>
                                                                            </div><!-- End .cart-product-quantity -->
                                                                        </td>
                                                                        <td class="total-col">
                                                                            <?= "&#8358; " . $item_total ?>
                                                                        </td>
                                                                        <td class="remove-col"><a href="cart.php?remove=<?= $key ?>" class="btn-remove"><i class="icon-close"></i></a></td>
                                                                    </tr>
                                                            <?php }
                                                    } ?>
                                            <?php } ?>
                                ...
                                <div class="summary summary-cart">
                                    ...
                                    <table class="table table-summary">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="summary-total">
                                                <td>Total:</td>
                                                <td id="subtotal" orginal-subtotal=<?= $subtotal ?>><?= number_format($subtotal, 2) ?></td>
                                            </tr><!-- End .summary-total -->
                                        </tbody>
                                    </table><!-- End .table table-summary -->
