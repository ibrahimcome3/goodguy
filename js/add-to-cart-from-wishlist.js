            ...
            $('.page-wrapper').on('click', '.submit-cart', function (e) {
                ...
                var $qtyInput = $row.find('#qty-' + inventoryItemId); // Targets specific qty input by ID
                
                var quantity = 1; // Default quantity
                if ($qtyInput.length > 0) {
                    var parsedQty = parseInt($qtyInput.val(), 10);
                ...
                $.ajax({
                   type: "POST", url: 'cart_ajax.php', // Ensure this is the correct path
                    data: { inventory_product_id: inventoryItemId, qty: quantity }, // Use dynamic quantity
                    dataType: 'json',
