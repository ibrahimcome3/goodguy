														...
														while ($row = $stmt->fetch()) {
															...
															<tr>
																<td><a style="color: blue"
																		href='order_detail.php?order_id=<?= $row['order_id'] ?>'><?= $row['order_id'] ?></a>
																</td>
																...
																<td><a onclick="return confirm('Are you sure?');"
																		href="cancel-order.php?order_id=<?= $row['order_id'] ?>">Cancel</a>
																</td>
															</tr>
														<?php } ?>
													</tbody>
										...
										while ($row = $stmt->fetch()) {
											...
											<ul>
												<li phone-id="<?= $row['phone_id'] ?>">
													<p class="phon widget-title" phone-number=<?= $row['PhoneNumber'] ?>><?= $row['PhoneNumber'] ?> <span
															style="color: green; margin-left: 10px;"><?php if ($row['default_'] === 1) {
																echo "&#10004;";
															} ?></span>
													</p>
													<p><a class="edit-phone-number" phone-id="<?= $row['phone_id'] ?> "
															<?php echo 'href="edit-phone-number-page.php?phone=' . $ph . '&phone_id=' . $phid . '"'; ?>>Edit</a>' &#8729; <a
															href="delete_phone_number.php?phone_id=<?= $row['phone_id'] ?>"
															phone-id="<?= $row['CustomerID'] ?> ">Delete</a> &#8729; <a
															href="make-phone-number-default.php?phone_id=<?= $row['phone_id'] ?>"
														...
														<div class="card card-dashboard">
															<div class="card-body">
																...
																<p><?= $row['address1'] ?><br>
																	...
																	<a
																		href="edit-shipping_address.php?sno=<?= $row['shipping_address_no'] ?>">Edit</a>
																</p>
															</div><!-- End .card-body -->
										...
										<form action="update_customer_records.php" method="POST">
											<div class="row">
												<div class="col-sm-6">
													...
													<input value="<?= $user__['customer_fname'] ?>" type="text"
														name="fname" class="form-control" required>
												</div><!-- End .col-sm-6 -->

												<div class="col-sm-6">
													<label>Last Name *</label>
													<input type="text" value="<?= $user__['customer_lname'] ?>"
